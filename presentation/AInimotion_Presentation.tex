...
\begin{itemize}
    \item Random $256 \times 256$ crops
...
\begin{itemize}
    ...
    \item \textbf{Foreground (AdaCoF):} $7\times7$ deformable sampling kernels per pixel (49 weights + 98 offsets = 147 params/pixel)
...
\footnotesize
\[
\mathcal{L}_G = \underbrace{2.0 \cdot \mathcal{L}_{L1}}_{\text{Reconstruction}} + \underbrace{0.05 \cdot \mathcal{L}_{\text{perc}}}_{\text{VGG19}} + \underbrace{1.0 \cdot \mathcal{L}_{\text{edge}}}_{\text{Ink Lines}} + \underbrace{0.005 \cdot \mathcal{L}_{\text{GAN}}}_{\text{Phase 2}}
\]
\normalsize
...
\vspace{0.15cm}
\textbf{Early Results (Epochs 0--9):}
\begin{itemize}
    \item PSNR: $20.7 \rightarrow 22.1$ dB (rising)
...
\textbf{Next steps:}
\begin{itemize}
    \item Complete Phase 1 training (35 epochs)
    \item Hyperparameter sweep for heavy motion
