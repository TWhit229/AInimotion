...
% ==============================
% Slide 10: Training Iterations & Experiments
% ==============================

\begin{frame}{Training Iterations \& Experimentation}
\scriptsize
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\textbf{Phase 1: Prototype} \hfill \textit{32ch, K=7}\\
Gradient explosion $\to$ added clipping + AMP\\[3pt]
\textbf{Phase 2: Scale-Up} \hfill \textit{48ch, K=7}\\
28.7\,dB test PSNR; D dominance emerged\\[3pt]
\textbf{Phase 3: Probes} \hfill \textit{4 experiments}\\
Weakening D helps, but not alone\\[3pt]
\textbf{Phase 4: Extensions} \hfill \textit{5 experiments}\\
Higher GAN weight $\to$ instability; throttling wins
\end{column}
\begin{column}{0.48\textwidth}
\textbf{Phase 5: Edge-Aware} \hfill \textit{64ch, K=7}\\
Adaptive lr\_d + $20\times$ edge mult $\to$ 23.9\,dB\\[3pt]
\textbf{Phase 6: Sweep} \hfill \textit{96ch, 13 configs}\\
K=9, crop 384, perc 0.1 wins at 21.8\,dB\\[3pt]
\textbf{Phase 7: Final Run} \hfill \textit{96ch, K=9}\\
Gradient accumulation; 22.1\,dB at epoch 7\\[3pt]
\textbf{Hardware:} 5070\,Ti (16\,GB) $\to$ 5090 (32\,GB)
\end{column}
\end{columns}
...
\begin{columns}
\begin{column}{0.48\textwidth}
\begin{block}{Scale of Exploration}
\centering
\textbf{30+ configurations} across \textbf{7 phases}\\
4 capacity levels $\cdot$ 9 GAN experiments $\cdot$ 13-run sweep
\end{block}
\end{column}
\begin{column}{0.48\textwidth}
\begin{block}{Key Lesson}
\centering
GAN training in anime VFI is highly\\
sensitive --- systematic search is essential
\end{block}
\end{column}
\end{columns}
\end{frame}
