...
\begin{column}{0.55\textwidth}
\footnotesize
\begin{tabular}{@{}p{1.7cm}p{1.5cm}p{2.2cm}@{}}
\toprule
\textbf{Phase} & \textbf{Config} & \textbf{Key Insight} \\
\midrule
Prototype & 32ch, K=7 & Gradient explosion $\to$ added clipping \\
Scale-up & 48ch, K=7 & 28.7 dB test; D dominance issue \\
Probes (4) & 48ch varied & D weakening helps but not enough \\
Extensions (5) & 32ch/48ch & Higher GAN weight $\to$ instability \\
Phase 3 & 64ch, K=7 & Edge-aware + adaptive lr\_d \\
GPU Upgrade & 5070 Ti $\to$ 5090 & $2\times$ VRAM (16$\to$32 GB) \\
Sweep (13) & 96ch varied & K=9, crop 384, perc 0.1 wins \\
\textbf{Final} & \textbf{96ch, K=9} & \textbf{In progress (ep 7+)} \\
\bottomrule
\end{tabular}
\end{column}
\begin{column}{0.42\textwidth}
...
\begin{itemize}
    \item \textbf{4 capacity levels} (32$\to$48$\to$64$\to$96 channels)
    \item \textbf{9 GAN balance experiments} to tame discriminator
    \item \textbf{13-experiment sweep} covering lr, loss weights, kernel, crop size
    \item \textbf{Gradient accumulation} for large crops on 32\,GB VRAM
\end{itemize}

\vspace{0.15cm}
\begin{block}{Total Experiments}
\centering
\textbf{30+ configurations tested}\\
